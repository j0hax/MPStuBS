...
struct system_configuration {
  ...
  system_configuration() : cpu_count(0), bsp_id(0), ioapic_id(-1),
    disablePICs(false) {
    for (int i = 0; i < 24; ++i) {
      ioapic_irqs[i] = -1;
...
private:
  ...
  unsigned char
  logicalLAPICID[CPU_MAX]; ///< logical LAPIC IDs per CPU, needed for IPI
...
private:
  ...
  // finishes IRQ Map initialisation after ACPI/MPS detection
