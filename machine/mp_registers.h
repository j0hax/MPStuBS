...
struct mpfps {
  ...
  unsigned char
  feature1; ///< =0: there is an MP config table; otherwise: default config
  unsigned char
  feature2; ///< bit 7 set: IMCR and PIC mode; otherwise: virtual wire mode
...
static_assert(sizeof(struct mpct_processor) == 4 * 5,
              "wrong size for mpct_processor");
...
static_assert(sizeof(struct mpct_ioapic) == 4 * 2,
              "wrong size for mpct_ioapic");
