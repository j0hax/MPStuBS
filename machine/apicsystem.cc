      ...
      switch (entry_type) {
        ...
        case APIC_Structure::SOURCE_OVERRIDE: {
          ...
          if (override->getBus() != 0) {
            DBG_VERBOSE << "Found override for bus != ISA. Does not conform to ACPI." <<
                        endl;
            break;
          }
...
bool APICSystem::searchConfig(unsigned long base, unsigned long length,
                              SystemConfig &conf) {
  struct mpfps* mpfps;
  DBG_VERBOSE << "searching MPFP structure starting at " << hex << base <<
  ...
  for (; length > 0; bp += 4, length -= 16) {
    ...
    if (mpfps->length != 1) {
      ...
      continue;
      // checksum OK?
    } else if (!mpbChecksumOK((unsigned char*) bp,
                              mpfps->length * sizeof(struct mpfps))) {
      DBG_VERBOSE << "invalid checksum" << endl;
      continue;
      // valid version of the MP spec?
    } else if (mpfps->spec_rev != 1 && mpfps->spec_rev != 4) {
      DBG_VERBOSE << "unknown MPFS spec_rev = " << hex << (int)mpfps->spec_rev << dec
                  << endl;
      continue;
    }
    ...
    switch (*mpt) {
      case MPCT_PROCESSOR: {
        ...
        DBG_VERBOSE << "found processor with lapicid " << (int) m->lapicid << " CPU "
                    << ((m->cpuflag & CPU_ENABLED) ? ((char*) " enabled") : ((
                          char*) " disabled")) << endl;
        break;
      }
...
//	}
  ...
  unsigned long start_eip = (unsigned long)
                            relocatedSetupAP; // address of relocated setup_ap()
